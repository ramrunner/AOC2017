...
(define (tokenize  lns)
  (map (lambda (ln) 
         (let ((strs (string-split ln "\t")))
           (map string->number strs)))
       (string-split lns "\n")))

;accumulate tha max and min from a list and return the diff
(define accumaxmin (lambda (ln)
        (letrec ((min 0)
                 (max 0)
                 (ftime #t)
                 (prevs '())
                 (recurse (lambda (lst) 
                             (if (null? lst)
                                 lst
                                 (begin
                                   (cond ((eq? ftime #t) (set! min (car lst)) (set! max min) (set! ftime #f))
				           ((< (car lst) min) (set! min (car lst)))
                                           ((> (car lst) max) (set! max (car lst))))
                                           (recurse (cdr lst)))))))
                                  
         (recurse ln)
         (- max min)
         (format #t "min: ~D max: ~D ~%" min max))))


;accumulate the exact dividends
(define accudivs (lambda (ln)
   (let ((res 0))
     (do-list i ln ;too lazy to not do a double for right now.
       (do-list j ln
         (if (and (not (eq? i j)) (eq? (modulo i j) 0))
               (begin 
                 (format #t "i found a pair ~D ~D ~%" i j) 
                 (set! res(/ i j))))))
    res)))

(display (foldr + 0 (map accudivs (tokenize myinput))))
